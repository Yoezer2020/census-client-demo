...
// Mock PaymentServiceType interface
  ...
  useEffect(() => {
    const fetchPaymentTypes = async () => {
      ...
      try {
        ...
        // Auto-select "RELATIONSHIP_APPLICATION" if not already selected
        const relationshipType = mockPaymentTypes.find(
          (type) => type.payment_type === "RELATIONSHIP_APPLICATION",
        );
        if (relationshipType && !value) {
          onSelect(relationshipType.id, relationshipType);
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-2 animate-in slide-in-from-top-2 duration-300">
      ...
      <p className="text-xs text-gray-500 pl-0.5">
        Auto-selected for relationship applications
      </p>
      {error && (
